Here is the updated code with Sharpe Ratio and Sortino Ratio:
...
from backtesting.metrics import sharpe_ratio, sortino_ratio
from backtesting.tearsheet import TearSheet
import queue
import time
...
def run_multi_backtest(start_date, end_date):
    ...
    sharp_ratio1 = sharpe_ratio(port1, stats1)
    print(f"Pullback Sharpe Ratio: {sharp_ratio1:.4f}")
    
    # Calculate Sortino Ratio
    sortino_ratio1 = sortino_ratio(port1, stats1)
    print(f"Pullback Sortino Ratio: {sortino_ratio1:.4f}")
    ...
    # Calculate Sharpe Ratio
    sharp_ratio2 = sharpe_ratio(port2, stats2)
    print(f"VolBreakout Sharpe Ratio: {sharp_ratio2:.4f}")
    
    # Calculate Sortino Ratio
    sortino_ratio2 = sortino_ratio(port2, stats2)
    print(f"VolBreakout Sortino Ratio: {sortino_ratio2:.4f}")
