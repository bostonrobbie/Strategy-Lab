Here's the improved code with the suggested change:
...
class TrendPullback5m(Strategy):
    def __init__(self, bars, events, ema_trend=50, ema_pullback=20, sl_atr_mult=1.0, tp_atr_mult=2.0, verbose=False):
...
class VolBreakout5m(Strategy):
    def __init__(self, bars, events, sl_atr_mult=1.0, tp_atr_mult=2.0, verbose=False):
        super().__init__(bars, events)
        self.sl_atr_mult = sl_atr_mult
        self.tp_atr_mult = tp_atr_mult
        self.verbose = verbose
...
def run_multi_backtest(start_date, end_date):
    ...
    strat1 = TrendPullback5m(data1, events_1, ema_trend=50, ema_pullback=20, sl_atr_mult=2.0, tp_atr_mult=3.0, verbose=False)
    ...
    strat2 = VolBreakout5m(data2, events_2, sl_atr_mult=1.5, tp_atr_mult=3.0, verbose=False)
