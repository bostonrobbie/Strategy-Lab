...
def load_data(symbol='NQ', interval='5m', start_date='2020-01-01', end_date='2024-12-31'):
    """Load data efficiently with date filtering and validate timestamps"""
    ...
    # Validate timestamps: no gaps allowed!
    if not (df.index.isin(df.index.shift(1)).all()):
        print("Error: Timestamps contain gaps!")
        return None
