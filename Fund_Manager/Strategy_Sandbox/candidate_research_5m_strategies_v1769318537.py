Here is the modified code with the suggested improvement:
...
# ==========================================
# 2. Volatility Breakout with Bollinger Bands (5m)
# ==========================================
class VolBreakoutBands5m(Strategy):
    ...
    def calculate_signals(self, event):
        ...
        # Entry Logic: Inside Bar with Bollinger Bands
        ...
        bb_upper = s.ewm(span=20, adjust=False).mean() + 2 * s.ewm(span=20, adjust=False).std()
        bb_lower = s.ewm(span=20, adjust=False).mean() - 2 * s.ewm(span=20, adjust=False).std()

        if is_inside:
            # Check Breakout in Current Bar
            # Breakout Long (Only if Trend is UP)
            if bar_current.close > bb_upper.iloc[-1] and bar_current.close > ema_trend:
            ...
            elif bar_current.close < bb_lower.iloc[-1] and bar_current.close < ema_trend:
...
def run_multi_backtest(start_date, end_date):
    ...
    # --- Strategy 2: Volatility Breakout with Bollinger Bands ---
    print("\n>>> Testing Volatility Breakout with Bollinger Bands (5m)...")
    ...
    strat2 = VolBreakoutBands5m(data2, events_2, verbose=False)
    ...
    if ret1 > ret2:
        ...
        print("\nWinner: Volatility Breakout with Bollinger Bands")
