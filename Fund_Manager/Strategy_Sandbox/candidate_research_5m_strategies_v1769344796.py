...
# ==========================================
# 1. Adaptive Trend Pullback (5m)
# =================================---------
class AdaptiveTrendPullback5m(Strategy):
    def __init__(self, bars, events, ema_trend_len=50, ema_pb_len=20, sl_atr_mult=2.0, tp_atr_mult=4.0, verbose=False):
        super().__init__(bars, events)
        self.ema_trend_len = ema_trend_len
        self.ema_pb_len = ema_pb_len
...
def run_multi_backtest(start_date, end_date):
    ...
    # --- Strategy 1: Adaptive Trend Pullback ---
    print("\n>>> Testing Adaptive Trend Pullback (5m)...")
    ...
    strat1 = AdaptiveTrendPullback5m(data1, events_1, verbose=False)
    ...
    ts1 = TearSheet(port1)
    stats1 = ts1.analyze()
    print(f"Adaptive Trend Pullback Return: {stats1.get('Total Return', 0):.2%}")
    print(f"Adaptive Trend Pullback MaxDD:  {stats1.get('Max Drawdown', 0):.2%}")
    ...
    if ret1 > ret2:
        print("\nWinner: Adaptive Trend Pullback")
    else:
