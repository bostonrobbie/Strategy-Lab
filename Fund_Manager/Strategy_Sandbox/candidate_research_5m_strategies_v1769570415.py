...
def run_multi_backtest(start_date, end_date):
    ...
    for name in ["Strategy 1", "Strategy 2"]:
        ...
        # Validate data
        if not all(df['Timestamp'].map(lambda x: isinstance(x, datetime))):
            print("[-] Error: Invalid timestamp format. Skipping.")
            continue
        
        for m in df.groupby(pd.Grouper(freq='5min')):
            if len(m[1]) < 2:
                print(f"[-] Error: {name} has less than 2 trades. Skipping.")
                continue
            ...
            print(f"{name}: {status}")
            if pass_count > 0:
                print(f"  [+] Passes: {', '.join([str(x) for x in range(pass_count)])}")
            if fails:
