Here is the updated code with parameterized SMA crossover levels:
...
class TrendPullback5m(Strategy):
    def __init__(self, bars, events, sma_short=50, sma_long=200, ema_trend=50, ema_pullback=20, sl_atr=2.0, tp_atr=4.0, verbose=False):
...
class VolBreakout5m(Strategy):
    def __init__(self, bars, events, sma_short=50, sma_long=200, sl_atr=1.5, tp_atr=3.0, verbose=False):
        super().__init__(bars, events)
        self.sma_short = sma_short
        self.sma_long = sma_long
        self.sl_atr_mult = sl_atr
    ...
    def calculate_signals(self, event):
        ...
        is_inside = (bar_inside.high < self.sma_long and bar_inside.low > self.sma_short)
        ...
        if is_inside:
            ...
            if bar_current.close > self.sma_long and bar_current.close > ema_trend:
            ...
            elif bar_current.close < self.sma_long and bar_current.close < ema_trend:
...
def run_multi_backtest(start_date, end_date):
    ...
    strat1 = TrendPullback5m(data1, events_1, sma_short=50, sma_long=200, ema_trend=50, ema_pullback=20, sl_atr=2.0, tp_atr=4.0, verbose=False)
    ...
    strat2 = VolBreakout5m(data2, events_2, sma_short=50, sma_long=200, sl_atr=1.5, tp_atr=3.0, verbose=False)
