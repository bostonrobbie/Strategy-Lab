Here is the modified code with the adaptive stop-loss for Volatility Breakout strategy:
...
def run_multi_backtest(start_date, end_date):
    ...
    # --- Strategy 2: Volatility Breakout with Adaptive Stop-Loss ---
    ...
    def adaptive_sl_atr(event):
        if self.prev_high is None:
            return
        atr = tr[-14:].mean()
        sl = self.entry_price - (atr * 1.5)
        tp = self.entry_price + (atr * 3.0)
        
    strat2.adaptive_sl_atr = adaptive_sl_atr
