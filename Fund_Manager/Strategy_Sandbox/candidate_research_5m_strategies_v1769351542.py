...
class TrendPullback5m(Strategy):
    ...
    def position_sizing(self, bars, symbol):
        ...
        if atr > 2.0: # High volatility, reduce position size
            return 0.5
        elif atr > 1.5: 
            return 0.8
        else:
    ...
    def calculate_signals(self, event):
        ...
        if is_inside:
            ...
            if bar_current.close > bar_inside.high and bar_current.close > ema_trend:
                 ...
                 self.buy(symbol, 1 * self.position_sizing(bars, symbol))
            ...
            # Breakout Short (Only if Trend is DOWN)
                 ...
                 self.sell(symbol, 1 * self.position_sizing(bars, symbol))
                 self.entry_price = bar_current.close
                 self.current_pos = -1
