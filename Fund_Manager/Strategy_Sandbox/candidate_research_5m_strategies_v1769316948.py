Here is the improved strategy code with additional metrics for evaluating the performance:
...
def run_multi_backtest(start_date, end_date):
    ...
    sharpe_ratio_1 = (stats1.get('Annualized Total Return', 0) - stats1.get('Risk-Free Rate', 0)) / np.sqrt(stats1.get('Volatility', 0))
    sortino_ratio_1 = (stats1.get('Annualized Total Return', 0) - stats1.get('Risk-Free Rate', 0)) / np.sqrt(stats1.get('Max Drawdown', 0))
    
    sharpe_ratio_2 = (stats2.get('Annualized Total Return', 0) - stats2.get('Risk-Free Rate', 0)) / np.sqrt(stats2.get('Volatility', 0))
    sortino_ratio_2 = (stats2.get('Annualized Total Return', 0) - stats2.get('Risk-Free Rate', 0)) / np.sqrt(stats2.get('Max Drawdown', 0))
    
    if sharpe_ratio_1 > sharpe_ratio_2 and sortino_ratio_1 > sortino_ratio_2:
        print("\nWinner: Trend Pullback (based on Sharpe Ratio)")
    elif sharpe_ratio_1 < sharpe_ratio_2 and sortino_ratio_1 < sortino_ratio_2:
        print("\nWinner: Volatility Breakout (based on Sharpe Ratio)")
    else:
        print("\nTie")
